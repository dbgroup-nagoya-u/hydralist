...
void
N::upgradeToWriteLockOrRestart(uint64_t &version, bool &needRestart)
{
...
inline N *
N::getChild(const uint8_t k, const N *node)
{
  switch (node->getType()) {
    case NTypes::N4: {
      auto n = static_cast<const N4 *>(node);
...
uint64_t
N::readLockOrRestart(bool &needRestart) const
{
  uint64_t version;
...
void
N::checkOrRestart(uint64_t startRead, bool &needRestart) const
{
  readUnlockOrRestart(startRead, needRestart);
}

void
N::readUnlockOrRestart(uint64_t startRead, bool &needRestart) const
{
  needRestart = (startRead != typeVersionLockObsolete.load());
}

uint32_t
N::getPrefixLength() const
{
  return prefixCount;
}
...
TID
N::getAnyChildTid(const N *n, bool &needRestart)
{
  const N *nextNode = n;
