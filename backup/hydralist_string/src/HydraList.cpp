...
void
HydraListImpl::createWorkerThread(int numNuma)
{
...
void
HydraListImpl::createCombinerThread()
{
...
#ifdef HYDRALIST_ENABLE_STATS
  std::cout << "total_dl_time: " << total_dl_time / 112 / 1000 << std::endl;
  std::cout << "total_sl_time: " << total_sl_time / 112 / 1000 << std::endl;
#endif
}

ListNode *
HydraListImpl::getJumpNode(Key_t &key)
{
...
bool
HydraListImpl::insert(Key_t &key, Val_t val)
{
...
bool
HydraListImpl::update(Key_t &key, Val_t val)
{
  uint64_t clock = ordo_get_clock();
  curThreadData->read_lock(clock);
...
Val_t
HydraListImpl::lookup(Key_t &key)
{
  /*
  if (!g_globalStop) {
      g_globalStop = true;
...
bool
HydraListImpl::remove(Key_t &key)
{
  uint64_t clock = ordo_get_clock();
  curThreadData->read_lock(clock);
...
SearchLayer *
HydraListImpl::createSearchLayer()
{
...
uint64_t
HydraListImpl::scan(Key_t &startKey, int range, std::vector<Val_t> &result)
{
  return dl.scan(startKey, range, result, getJumpNode(startKey));
}
void
HydraListImpl::registerThread()
{
...
void
HydraListImpl::unregisterThread()
{
