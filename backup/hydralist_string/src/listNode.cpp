...
void
ListNode::setMin(Key_t key)
{
  this->min = key;
}

void
ListNode::setPrefixLength(uint8_t pl)
{
  this->prefixLength = pl;
}

void
ListNode::setNumEntries(int numEntries)
{
  this->numEntries = numEntries;
}

void
ListNode::setDeleted(bool deleted)
{
...
bool
ListNode::readUnlock(version_t oldVersion)
{
  return verLock.read_unlock(oldVersion);
}

void
ListNode::writeUnlock()
{
  return verLock.write_unlock();
}

int
ListNode ::getNumEntries()
{
  return this->numEntries;
}

Key_t *
ListNode ::getKeyArray()
{
  return this->keyArray;
}

Val_t *
ListNode ::getValueArray()
{
...
uint8_t
LCPlength(const char *str1, const char *str2)
{
  int ret = 0;
...
ListNode ::split(Key_t &key, Val_t val, uint8_t keyHash)
{
  ...
  if (key < newMin) {
    ...
    newNode->insertAtIndex(key, val, newNumEntries, keyHash);
...
bool
ListNode ::insertAtIndex(const Key_t &key, Val_t value, int index, uint8_t keyHash)
{
  keyArray[index] = key;
...
bool
ListNode ::updateAtIndex(Val_t value, int index)
{
  valueArray[index] = value;
  return true;
}
...
bool
ListNode ::removeFromIndex(int index)
{
  bitMap.reset(index);
  numEntries--;
  return true;
...
uint8_t
ListNode ::getKeyInsertIndex(const Key_t &key)
{
...
#if MAX_ENTRIES == 64
int
ListNode::getKeyIndex(const Key_t &key, uint8_t keyHash)
{
  ...
  while (posToCheck) {
    ...
    // printf("pos: %d key: %d\n", pos, key);
    if (memcmp(&keyArray[pos], key.getData(), 32) == 0) return pos;
...
int
ListNode::getFreeIndex(Key_t &key, uint8_t keyHash)
{
  int freeIndex;
...
ListNode::getKeyIndex(const Key_t &key, uint8_t keyHash)
{
  ...
  while (posToCheck) {
    ...
    asm("bsrl %1, %0" : "=r"(pos) : "r"((uint32_t)posToCheck));
    // printf("pos: %d key: %d\n", pos, key);
    if (keyArray[pos] == key) return pos;
    posToCheck = posToCheck & (~(1 << pos));
  }
  posToCheck = (uint32_t)(posToCheck_64 >> 32);
  while (posToCheck) {
    ...
    asm("bsrl %1, %0" : "=r"(pos) : "r"((uint32_t)posToCheck));
    // printf("xxxpos: %d key: %d\n", pos, key);
    if (keyArray[pos + 32] == key) return pos + 32;
    posToCheck = posToCheck & (~(1 << pos));
  }
  ...
  while (posToCheck) {
    ...
    asm("bsrl %1, %0" : "=r"(pos) : "r"((uint32_t)posToCheck));
    // printf("pos: %d key: %d\n", pos, key);
    if (keyArray[pos + 64] == key) return pos + 64;
    posToCheck = posToCheck & (~(1 << pos));
  }
  posToCheck = (uint32_t)(posToCheck_64 >> 32);
  while (posToCheck) {
    ...
    asm("bsrl %1, %0" : "=r"(pos) : "r"((uint32_t)posToCheck));
    // printf("xxxpos: %d key: %d\n", pos, key);
    if (keyArray[pos + 96] == key) return pos + 96;
    posToCheck = posToCheck & (~(1 << pos));
  }
  return -1;
}
...
ListNode::getKeyIndex(const Key_t &key, uint8_t keyHash)
{
  int count = 0;
  for (uint8_t i = 0; i < MAX_ENTRIES; i++) {
    if (bitMap[i] && keyHash == fingerPrint[i] && keyArray[i] == key) return (int)i;
    if (bitMap[i]) count++;
...
ListNode::getFreeIndex(Key_t &key, uint8_t keyHash)
{
  ...
  for (uint8_t i = 0; i < MAX_ENTRIES; i++) {
    if (keyCheckNeeded && bitMap[i] && keyHash == fingerPrint[i] && keyArray[i] == key) return -1;
...
bool
ListNode::insert(Key_t &key, Val_t value)
{
  ...
  if (!insertAtIndex(key, value, (uint8_t)index, keyHash)) return false;
  return true;
}

bool
ListNode::update(Key_t &key, Val_t value)
{
  uint8_t keyHash = getKeyFingerPrint(key);
...
bool
ListNode ::checkRangeLookup(Key_t &key)
{
  int ne = numEntries;
...
bool
ListNode::lookup(Key_t &key, Val_t &value)
{
  int keyHash = getKeyFingerPrint(key);
  int index = getKeyIndex(key, keyHash);
  if (index == -1)
...
bool
ListNode::scan(Key_t &startKey, int range, std::vector<Val_t> &rangeVector, uint64_t writeVersion)
{
...
uint8_t
ListNode::getKeyFingerPrint(const Key_t &key)
{
...
uint8_t *
ListNode::getFingerPrintArray()
{
  return fingerPrint;
}
bool
compare(std::pair<Key_t, uint8_t> &x, std::pair<Key_t, uint8_t> &y)
{
  return x.first < y.first;
}
void
ListNode::generatePermuter(uint64_t writeVersion)
{
  pLock.lock();
  if (writeVersion == lastScanVersion) {
    pLock.unlock();
...
int
ListNode ::permuterLowerBound(Key_t &key)
{
  int lower = 0;
  int upper = numEntries;
  do {
