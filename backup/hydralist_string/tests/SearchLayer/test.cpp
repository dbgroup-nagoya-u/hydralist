...
void
insert(
    SearchLayer *sl, int threadId, int totalInserts, int MAX_THREADS, std::vector<uint64_t> *keyptr)
{
...
void
insert1(SearchLayer *sl, int threadId, int totalInserts, int MAX_THREADS, uint64_t *keys)
{
  int range = totalInserts / MAX_THREADS;
  for (unsigned long i = threadId * range; i < (threadId + 1) * range; i++) {
    if (i % 8 == 0) sl->insert(keys[i], (void *)&keys[i]);
  }
}
void
lookup(
...
TEST(searchLayerTest, sanityTest)
{
  ...
  for (uint64_t i = 0; i < totalInserts; i++) keys[i] = i;
...
TEST(searchLayerTest, powerOf2Test)
{
  ...
  std::vector<uint64_t> keys(totalInserts);
  for (uint64_t i = 1; i < totalInserts; i++) keys[i] = i;
  for (int i = 0; i < MAX_THREADS; i++) {
    threads[i] = new std::thread(insert, sl, i, totalInserts, MAX_THREADS, &keys);
  }
  for (int i = 0; i < MAX_THREADS; i++) {
