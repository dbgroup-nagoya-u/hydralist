...
std::atomic<N *> *
N16::getChildPos(const uint8_t k)
{
...
N *
N16::getChild(const uint8_t k) const
{
  __m128i cmp = _mm_cmpeq_epi8(_mm_set1_epi8(flipSign(k)),
                               _mm_loadu_si128(reinterpret_cast<const __m128i *>(keys)));
...
bool
N16::remove(uint8_t k, bool force)
{
  if (count == 3 && !force) {
    return false;
  }
...
N *
N16::getSmallestChild(uint8_t start) const
{
...
N *
N16::getLargestChild(uint8_t end) const
{
