...
TID
Tree::lookup(const Key &k, ThreadInfo &threadEpocheInfo) const
{
...
bool
Tree::lookupRange(const Key &start,
                  const Key &end,
                  Key &continueKey,
                  TID result[],
                  ...
                  ThreadInfo &threadEpocheInfo) const
{
  ...
  std::function<void(const N *, uint32_t)> findStart =
      [&copy, &start, &findStart, &toContinue, &restart, this](const N *node, uint32_t level) {
        if (N::isLeaf(node)) {
          copy(node);
          return;
        }
  ...
  std::function<void(const N *, uint32_t)> findEnd = [&copy, &end, &toContinue, &restart, &findEnd,
...
TID
Tree::checkKey(const TID tid, const Key &k) const
{
  Key kt;
  this->loadKey(tid, kt);
  if (k == kt) {
...
void
Tree::insert(const Key &k, TID tid, ThreadInfo &epocheInfo)
{
...
restart:
  ...
  while (true) {
    ...
    if (nextNode == nullptr) {
      ...
      N::insertAndUnlock(node, parentNode, parentKey, nodeKey, N::setLeaf(tid), epocheInfo,
                         needRestart);
      if (needRestart) goto restart;
      return;
    }
...
void
Tree::remove(const Key &k, TID tid, ThreadInfo &threadInfo)
{
...
typename Tree::CheckPrefixResult
Tree::checkPrefix(N *n, const Key &k, uint32_t &level)
{
...
Tree::checkPrefixPessimistic(N *n,
                             const Key &k,
                             uint32_t &level,
                             uint8_t &nonMatchingKey,
    ...
    for (uint32_t i = ((level + p.prefixCount) - n->getLevel()); i < p.prefixCount; ++i) {
      ...
      if (curKey != k[level]) {
        ...
        if (p.prefixCount > maxStoredPrefixLength) {
          ...
          for (uint32_t j = 0;
               j < std::min((p.prefixCount - (level - prevLevel) - 1), maxStoredPrefixLength);
               ++j) {
...
Tree::checkPrefixCompare(const N *n, const Key &k, uint32_t &level, LoadKeyFunction loadKey)
{
  Prefix p = n->getPrefi();
  if (p.prefixCount + level < n->getLevel()) {
...
Tree::checkPrefixEquals(
    const N *n, uint32_t &level, const Key &start, const Key &end, LoadKeyFunction loadKey)
{
  Prefix p = n->getPrefi();
  if (p.prefixCount + level < n->getLevel()) {
...
TID
Tree::lookupNext1(const Key &start, ThreadInfo &threadEpocheInfo) const
{
  EpocheGuardReadonly epocheGuard(threadEpocheInfo);
  TID toContinue = 0;
  bool restart;
...
TID
Tree::lookupNext(const Key &start, ThreadInfo &threadEpocheInfo) const
{
  EpocheGuardReadonly epocheGuard(threadEpocheInfo);
  uint32_t level = 0;
  N *node = root;
