...
void
N::writeLockOrRestart(bool &needRestart)
{
  uint64_t version;
  do {
...
void
N::lockVersionOrRestart(uint64_t &version, bool &needRestart)
{
...
N *
N::getAnyChildReverse(const N *node)
{
  switch (node->getType()) {
    case NTypes::N4: {
      auto n = static_cast<const N4 *>(node);
...
void
N::change(N *node, uint8_t key, N *val)
{
  switch (node->getType()) {
    case NTypes::N4: {
...
N *
N::getChild(const uint8_t k, N *node)
{
  switch (node->getType()) {
    case NTypes::N4: {
      auto n = static_cast<N4 *>(node);
...
bool
N::isLocked(uint64_t version) const
{
...
bool
N::isObsolete(uint64_t version)
{
  return (version & 1) == 1;
}

bool
N::checkOrRestart(uint64_t startRead) const
{
  return readUnlockOrRestart(startRead);
}

bool
N::readUnlockOrRestart(uint64_t startRead) const
{
...
void
N::setPrefix(const uint8_t *prefix, uint32_t length)
{
  if (length > 0) {
    Prefix p;
...
void
N::addPrefixBefore(N *node, uint8_t key)
{
...
std::tuple<N *, uint8_t>
N::getSecondChild(N *node, const uint8_t key)
{
  switch (node->getType()) {
    case NTypes::N4: {
      auto n = static_cast<N4 *>(node);
...
TID
N::getAnyChildTidReverse(const N *n)
{
  const N *nextNode = n;
...
N *
N::getSmallestChild(const N *node, uint8_t start)
{
  switch (node->getType()) {
    case NTypes::N4: {
      auto n = static_cast<const N4 *>(node);
...
N *
N::getLargestChild(const N *node, uint8_t start)
{
  switch (node->getType()) {
    case NTypes::N4: {
      auto n = static_cast<const N4 *>(node);
