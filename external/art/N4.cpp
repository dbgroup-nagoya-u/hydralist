...
N *
N4::getChild(const uint8_t k) const
{
...
bool
N4::remove(uint8_t k, bool /*force*/)
{
  for (uint32_t i = 0; i < compactCount; ++i) {
...
std::tuple<N *, uint8_t>
N4::getSecondChild(const uint8_t key) const
{
  for (uint32_t i = 0; i < compactCount; ++i) {
    N *child = children[i].load();
    if (child != nullptr) {
...
N *
N4::getSmallestChild(uint8_t start) const
{
...
N *
N4::getLargestChild(uint8_t end) const
{
